...
def get_client():
    global _client
    if _client is None:
        _client = genai.Client(enterprise=True)
    return _client
...
def get_image_embedding_from_multimodal_embedding_model(
    ...
    config = types.EmbedContentConfig(output_dimensionality=embedding_size)
    ...
    if return_array:
        return np.fromiter(image_embedding, dtype=float)
...
def get_text_overlapping_chunk(
    ...
    * Returns a dictionary where the keys are chunk numbers and the values are the corresponding text chunks.
    ...
    if overlap >= character_limit:
        raise ValueError("Overlap cannot be larger than or equal to character limit.")
...
def get_image_for_gemini(
    ...
    Returns:
    - Tuple[PIL_Image.Image, str]: A tuple containing the PIL Image object and the image filename.
    """
    ...
    image_for_gemini = PIL_Image.open(image_name)
...
def get_gemini_response(
    ...
    Args:
        model_input: A list of inputs to the model.
    ...
    if generation_config.safety_settings is None:
    ...
    for chunk in response:
        try:
            response_list.append(chunk.text or "")
...
def get_cosine_score(
    ...
    if val is None or (isinstance(val, (list, np.ndarray)) and len(val) == 0):
        return 0.0
    return float(np.dot(val, input_text_embed))
...
def print_text_to_image_citation(
    ...
    for imageno, image_dict in final_images.items():
        ...
        print(f"{Fore.BLUE}Score:{Style.RESET_ALL}", round(image_dict["cosine_score"], 2))
...
def print_text_to_text_citation(
    ...
    for textno, text_dict in final_text.items():
        ...
        # Print the cosine similarity score
        print(f"{Fore.BLUE}Score:{Style.RESET_ALL}", round(text_dict["cosine_score"], 2))
...
def get_similar_image_from_query(
    ...
    for matched_imageno, indexvalue in enumerate(top_n_cosine_scores):
        ...
        final_images[matched_imageno]["image_object"] = PIL_Image.open(
